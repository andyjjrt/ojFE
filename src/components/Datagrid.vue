<template>
  <v-sheet class="d-flex flex-column h-100">
    ...
    <div class="my-4 text-center" v-if="!loading && data.length === 0">No Data</div>
    <slot :data="data">
    ...
    <div class="d-flex justify-space-between align-center" v-if="!hidePagination">
      <div class="ms-3">
        <slot name="footer" />
      </div>
...
const totalPages = computed(() => Math.floor(props.total / props.rowsPerPage) + 1);
...
const handleChangeRowPerPage = (value: number) => emits("handleChangeRowPerPage", value);
</script>
